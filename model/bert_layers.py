...
class BertUnpadAttention(nn.Module):
    ...
    def forward(
        ...
        """
        self_output, self_attention = self.self(input_tensor, cu_seqlens, max_s, indices,
                                attn_mask, bias)
        if subset_idx is not None:
            return (self.output(index_first_axis(self_output, subset_idx),
                               index_first_axis(input_tensor, subset_idx)), self_attention)
        else:
            return (self.output(self_output, input_tensor), self_attention)
    ...
    def forward(
        ...
        **kwargs
        ...
        if masked_tokens_mask is None:
            ...
            raise NotImplementedError('Masked tokens not supported')
...
class BertForHierarchicalClassification(BertPreTrainedModel):
    def __init__(self, config, labels_maps, labels_weights=None):
        super().__init__(config)
        ...
        labels_sizes = [len(labels_map) for labels_map in labels_maps.values()]
        # Initialize the hierarchical classifier
        self.classifiers = nn.ModuleList([
            nn.Linear(config.hidden_size+labels_sizes[i-1], labels_sizes[i]) if i > 0 else nn.Linear(config.hidden_size, labels_sizes[i]) for i in range(len(labels_sizes))
        ])
    ...
    def forward(
        ...
        # labels (`torch.LongTensor` of shape `(batch_size, hierarchical_size)`, *optional*):
        ...
        pooled_output = self.dropout(outputs[1])  # CLS token representation
        
        # Get hierarchical logits
        logits = []
        for classifier in self.classifiers:
            logits.append(classifier(pooled_output))
            pooled_output = torch.cat((pooled_output, F.softmax(logits[-1], dim=1)), dim=1)
        ...
        if labels is not None:
            ...
            for i, logit in enumerate(logits):
                if self.labels_weights is None:
                    loss = nn.CrossEntropyLoss()(logit, labels[:, i])
                else:
                    weight_tensor = torch.tensor(self.labels_weights[i], dtype=logit.dtype).to(logit.device)
                    loss = nn.CrossEntropyLoss(weight=weight_tensor)(logit, labels[:, i])
